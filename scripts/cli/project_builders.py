...
def create_normal_project(_directory_path: str, **options: str):
    directory_path = Path(_directory_path)
...
def create_nodejs_project(_directory_path: str, **options: str):
    ...
    os.system(f"{js_pm.name} init -y {directory_path}")
...
def create_nodejsTS_project(_directory_path: str, **options: str):
    ...
    process = subprocess.Popen(
        f"""cd {directory_path} && {js_pm.package_manager} init -y""",
        shell=True,
    )
    process.wait()
    ...
    if js_pm.package_manager != "bun":
        ...
        os.system(
            f"""cd {directory_path} && {js_pm.dev_install_cmd} @types/node typescript ts-node && {js_pm.package_executor} tsc --init"""
        )
...
def create_nextjs_project(_directory_path: str, **options: str):
    ...
    os.system(f"{js_pm.name} create next-app@latest {directory_path}")
...
def create_rust_project(_directory_path: str, **options: str):
    directory_path = Path(_directory_path)
    os.system(f"cargo new {directory_path}")
