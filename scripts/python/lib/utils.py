...
def add_to_path(path: StrOrBytesPath):
    ...
    if not str(path) in str(previous_PATH):
        run_command(["setx", "Path", f"{previous_PATH};{path}"])
...
def extract_utilities(
    ...
    with zipfile.ZipFile(str(source)) as zip_ref:
        ...
        splitter = "/"
        directory = "bin"
        output_dir = destination / directory if with_bin else destination
        ...
        for name in zip_ref.namelist():
            if (directory + splitter) in name and not name.endswith(splitter):
                parts = name.split(splitter)

                if len(parts) > (steps + 1) and parts[steps] == directory:
                    filedata = zip_ref.read(name)
                    filepath = output_dir / splitter.join(parts[steps + 1 :])
